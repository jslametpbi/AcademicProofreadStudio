...
function portalUnlock(){
  const pinInput = $('portalPin');
  const val = (pinInput?.value || '').trim();
  if(val === ADMIN_PIN){
    grantAccess();
    sessionStorage.setItem('aps_admin','1');
    if(pinInput) pinInput.value = '';
    renderCurrentPage(document.querySelector('.page.active')?.id || 'home');
...
function bindPortalLogin(){
  const pin = $('portalPin');
  const btn = document.querySelector('.portal-minimal button');
  if(pin && !pin.dataset.bound){
    ...
    pin.addEventListener('input', ()=>{
      ...
      if(value === ADMIN_PIN) portalUnlock();
    });
    pin.addEventListener('focus', ()=>pin.placeholder='');
    pin.addEventListener('blur', ()=>{ if(!pin.value) pin.placeholder='PIN'; });
  }
...
document.addEventListener('keydown', (e)=>{
  if(e.key === 'Escape' && !$('accessPortal')?.classList.contains('hidden')){
    $('portalPin')?.focus();
  }
});
